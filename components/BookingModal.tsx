...
export default function BookingModal({ isOpen, onClose }: BookingModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center">
      ...
      {/* Modal Panel */}
      <div className="relative z-10 mx-2 w-full max-w-2xl max-h-[98vh] overflow-hidden rounded-2xl border border-gold/20 bg-background shadow-xl flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-white/[0.07] px-5 py-3 shrink-0">
          <h2 className="font-serif text-base font-bold text-white md:text-lg">
            Book Your Setup Call
          </h2>
          <button
            onClick={onClose}
            className="rounded-full p-1.5 text-subtle transition-colors hover:text-white"
            aria-label="Close"
          >
            <svg
              className="h-5 w-5"
        ...
        <div className="flex-1 min-h-0 relative overflow-y-auto" style={{ WebkitOverflowScrolling: "touch" }}>
          ...
          {/* Scroll hint */}
          <div className="sticky bottom-0 left-0 right-0 flex items-center justify-center gap-1.5 bg-gradient-to-t from-background via-background/95 to-transparent py-2.5 pt-6 pointer-events-none">
